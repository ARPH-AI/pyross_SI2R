...
cdef class SI2R(stochastic_integration):
    """
    Susceptible, Infected presymptomatic, Infected (asymptomatic and symptomatic), Removed (SI2R)
    ...
    * Is: symptomatic
    ...
    parameters: dict
        ...
        beta: float
            rate of spread of infection.
        gIp: float
            rate of removal from presymptomatic individuals.
        gIa: float
            rate of removal from asymptomatic individuals.
        gIs: float
            rate of removal from symptomatic individuals.
    ...
    """
    cdef:
        readonly double beta, gIp, gIa, gIs, fsa
        readonly np.ndarray xt0, Ni, dxtdt, lld, CC, alpha, population
    ...
    def __init__(self, parameters, M, Ni):
        ...
        self.nClass = 4 # Changed from 3 to 4 for SI2R
        alpha      = parameters['alpha']                    # fraction of asymptomatic infectives
        self.beta  = parameters['beta']                     # infection rate
        # Change for SI2R
        self.gIp   = parameters['gIp']                      # removal rate of Ip
        self.gIa   = parameters['gIa']                      # removal rate of Ia
        self.gIs   = parameters['gIs']                      # removal rate of Is
        self.fsa   = parameters['fsa']                      # the self-isolation parameter
        ...
        self.nReactions_per_agegroup = 5 # Changed from 3 to 5 for SI2R
        self.nReactions = self.M * self.nReactions_per_agegroup
        self.dim_state_vec = self.nClass * self.M
        ...
        for i in range(M):
            ...
            #self.vectors_of_change[  i*nRpa,i+  M] = +1
            # Change for SI2R
            # reaction S -> Ip at age group i:
            # population of S decreases by 1, population of Ip increases by 1
            self.vectors_of_change[  i*nRpa,i    ] = -1
            self.vectors_of_change[  i*nRpa,i+  M] = +1
            ...
            #self.vectors_of_change[1+i*nRpa,i+2*M] = +1
            # Change for SI2R
            # reaction Ip -> Is at age group i:
            # population of Ip decreases by 1, population of Ia increases by 1
            ...
            #self.vectors_of_change[1+i*nRpa,i+2*M] = +1
            # Change for SI2R
            # reaction Ip -> Ia at age group i:
            # population of Ip decreases by 1, population of Is increases by 1
            self.vectors_of_change[2+i*nRpa,i+  M] = -1
    ...
    cdef rate_vector(self, xt, tt):
        cdef:
            int N=self.N, M=self.M, i, j
            double  beta=self.beta, gIa=self.gIa, rateS, lmda
            double fsa=self.fsa, gIp=self.gIp, gIs=self.gIs
            #long [:] S    = xt[0  :M]
            #long [:] Ia   = xt[M  :2*M]
            #long [:] Is   = xt[2*M:3*M]
            # Change for SI2R
        ...
        for i in range(M): #, nogil=False):
            ...
            # Changes for SI2R
            for j in range(M): #, nogil=False):
                lmda += beta*(CM[i,j]*Ip[j]+CM[i,j]*Ia[j]+fsa*CM[i,j]*Is[j])/Ni[j]
            rateS = lmda*S[i] 
            #
            #rates[  i*nRpa] = alpha[i] *rateS     # rate S -> Ia
            #rates[1+i*nRpa] = (1-alpha[i]) *rateS # rate S -> Is
            # Changes for SI2R
            rates[  i*nRpa] = rateS                    # rate S  -> Ip
            rates[1+i*nRpa] = alpha[i] *     gIp*Ip[i] # rate Ip -> Ia
            rates[2+i*nRpa] = (1-alpha[i]) * gIp*Ip[i] # rate Ip -> Is
            #rates[2+i*nRpa] = gIa*Ia[i] # rate Ia -> R
            #rates[3+i*nRpa] = gIs*Is[i] # rate Is -> R
            # Changes for SI2R
    ...
    cpdef simulate(self, S0, Ip0, Ia0, Is0, contactMatrix, Tf, Nf,
                method='gillespie',
                int nc=30, double epsilon = 0.03,
                int tau_update_frequency = 1,
                ):
        ...
        out_dict = {'X':out_arr, 't':t_arr,
                     ...
                     'fsa':self.fsa,
                     'alpha':self.alpha, 'beta':self.beta,
                     'gIp':self.gIp, 'gIa':self.gIa, 'gIs':self.gIs}

        return out_dict



    cpdef simulate_events(self, S0, Ip0, Ia0, Is0, events,
                contactMatrices, Tf, Nf,
                method='gillespie',
                int nc=30, double epsilon = 0.03,
                int tau_update_frequency = 1,
        ...
        out_dict = {'X':out_arr, 't':t_arr,  'events_occured':events_out,
                     ...
                     'fsa':self.fsa,
                     'alpha':self.alpha, 'beta':self.beta,
                     'gIp':self.gIp, 'gIs':self.gIs, 'gIs':self.gIs}
        return out_dict
